...
- Mandatorio rodar em um subagent distinto a validacao para nao contaminar a validacao com o contexto de quem planejou e de quem implementou

## Fluxo Obrigatório

1. Identifique o escopo alterado com `git status`, `git diff --stat` e diffs relevantes.
2. Carregue o procedimento em `.claude/skills/ship/SKILL.md` e a skill de contexto da camada alterada (`arch-backend`, `arch-frontend`, `arch-data`…). As skills `workflow-prevc` e `*-especialista` NÃO existem no V8 — ver references/gates.md.
...
- Se subagents não estiverem disponíveis: PARE e reporte. No PREVEC V8 o review inline é proibido — 7 subagents distintos, nunca menos. Review inline contamina o contexto e é exatamente o que a fase V existe para evitar.
